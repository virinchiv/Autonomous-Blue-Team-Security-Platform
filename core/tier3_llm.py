...
def is_normal_linux_syslog(log: dict) -> bool:
    ...
    if message and re.search(r'(?i)session (opened|closed) for user (cyrus|news)', message):
        return True
    
    # Normal system services
    if message and re.search(r'(?i)(cupsd|syslogd).*(startup|shutdown|restart) succeeded', message):
        return True
    
    # Normal system processes
    if message and re.search(r'(?i)(logrotate|syslogd).*restart', message):
        return True
    
    # Normal FTP connections (not necessarily threats)
    if message and re.search(r'(?i)connection from.*at.*\d{4}', message):
        return True
...
def calculate_confidence_score(log: dict) -> float:
    ...
    try:
        ...
        if log_source == 'linux_syslog':
            ...
            if message and 'alert' in message.lower() and 'exited abnormally' in message.lower():
                score += 0.5
            
            # FTP timeouts and connection issues
            if message and ('timed out' in message.lower() or 'timeout' in message.lower()):
                score += 0.4
            
            # Privilege escalation attempts
            if message and 'session opened for user' in message.lower() and 'uid=0' in message:
                score += 0.3
            
            # Multiple connection attempts from same source
            source_ip = log.get('source.ip', '') or ''
        ...
        user_agent = log.get('user_agent.original', '') or ''
        ...
        if not user_agent or user_agent == '-':
            score += 0.3
        elif any(suspicious in user_agent_lower for suspicious in ['scanner', 'crawler', 'bot']):
            if not is_known_bot(user_agent):
                score += 0.4
        
        # Increase confidence for unusual URLs
        url = log.get('url.original', '') or ''
        if url and any(suspicious in url.lower() for suspicious in ['admin', 'wp-admin', 'phpmyadmin', '.env', 'config', 'backup']):
            score += 0.5
        
        # Special handling for SSRF rule - since it now only scans url.original, it's more reliable
        if url and any(ssrf_pattern in url.lower() for ssrf_pattern in ['http://', 'https://', 'ftp://', 'file://']):
            score += 0.3  # Boost confidence for SSRF patterns in actual URL
        
        return max(0.0, min(1.0, score))  # Clamp between 0 and 1
        
    except Exception as e:
        print(f"⚠️  Error in calculate_confidence_score: {e}")
